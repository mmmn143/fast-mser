...
namespace basicsys {
	...
	*/
	...
	public:
		...
		sys_json_writer& operator<<(const char* str);
		sys_json_writer& operator<<(const string& str);
		...
		template<class T>
		sys_json_writer& operator<<(const map<string, T>& val) {
			(*this)<<"{";
	...
	protected:
		...
		void text_write_str(const string& str);
		void binary_write_str(const string& str);

		void pre_process_map_seq(u8 type);
		void post_process_map_seq();

		b8 check_str(const string& str);
	...
	public:
		...
		sys_json_reader_iteartor(u8 node_type, map<string, sys_json_reader>::const_iterator& map_iterator, vector<sys_json_reader>::const_iterator seq_iterator) {
			m_node_type = node_type;
	...
	protected:
		...
		u8 m_node_type;
		map<string, sys_json_reader>::const_iterator m_map_iterator;
	...
	public:
		...
		sys_json_reader(sys_buffer_reader* reader, const string& node_name = "", i64 offset = 0, i32 size = -1, u8 node_type = sys_Node_Type_Map);

		sys_json_reader operator[](const i32 index) const;
		sys_json_reader operator[](const char* node_name) const {
			return (*this)[string(node_name)];
		}

		sys_json_reader operator[](const string& node_name) const;
		...
		b8 has_key(const string& key_name) const;
		...
		template<class T>
		void operator>>(map<string, T>& val) const {
			if (is_map()) {
				val.clear();
	...
	protected:
		...
		i32 find_match_character(const string& text, i8 need_matched_char, i32 start_index);

		map<string, sys_json_reader> m_map_nodes;
