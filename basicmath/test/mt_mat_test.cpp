...
protected:
	...
	static void performace_test_create_big_mat() {
		sys_timer big_create_single_channel("big_create_single_channel");
		...
		sys_timer big_create_multi_channel("big_create_multi_channel");
...
static void test_mat_add() {
	...
	sys_timer big_create_multi_channel("big_add");
	big_create_multi_channel.begin();
...
static void test_mat_switch_dim() {
	mt_mat a = mt_mat(2, 2, mt_F64C4).set_incremental(0, sys_false);

	basiclog_info2(a);

	vector<mt_mat> channels;
	a.all_channel(channels);

	mt_mat res = mt_mat_helper::merge_align_dim(channels, 1, sys_true);

	basiclog_info2(res);

	i32 input_channels = a.channel();

	i32 channel_size = 2;
	i32 sizes[] = {2, input_channels, channel_size};
	mt_mat delta = res.reshape(3, sizes, 1);

	basiclog_info2(delta);

	delta = delta.swap_dim(1, 2).clone();
	
	basiclog_info2(delta.last_dim_as_channel());
}

static void test_mat_flip() {
	basiclog_info2("test_mat_flip");
...
private:
	...
	static void multi_channel_test() {
		...
		mt_mat res = mat_33.channel_at(0).conv(mat_22.channel_at(0));
		mt_mat gt = mt_mat_t<f32>(2, 2, 1).read(20.0000000, 44.0000000, 92.0000000, 116.0000000);
		sys_test_equal(res, gt);

		res = mat_33.channel_at(1).conv(mat_22.channel_at(2));
		gt = mt_mat_t<f32>(2, 2, 1).read(36.0000000, 68.0000000, 132.0000000, 164.0000000);
		sys_test_equal(res, gt);

		res = mat_33.channel_at(0).conv(mat_22.channel_at(1));
		gt = mt_mat_t<f32>(2, 2, 1).read(28.0000000, 56.0000000, 112.0000000, 140.0000000);
		sys_test_equal(res, gt);

		res = mat_33.channel_at(1).conv(mat_22.channel_at(3));
		gt = mt_mat_t<f32>(2, 2, 1).read(44.0000000, 80.0000000, 152.0000000, 188.0000000);
		sys_test_equal(res, gt);

		res = mat_33.conv(mat_22);
		gt = mt_mat_t<f32>(2, 2, 2).read(56.0000000, 72.0000000, 112.0000000, 136.0000000, 224.0000000, 264.0000000, 280.0000000, 328.0000000);
		sys_test_equal(res, gt);

		mt_mat conv_res = res;
		res = mat_33.conv(conv_res, mt_Conv_Boundary_Type_Valid, mt_Conv_Kernel_Align_Type_Cross_Row);
		gt = mt_mat_t<f32>(2, 2, 4).read(784.0000000, 960.0000000, 784.0000000, 960.0000000, 1456.0000000, 1760.0000000, 1456.0000000, 1760.0000000, 2800.0000000, 3360.0000000, 2800.0000000, 3360.0000000, 3472.0000000, 4160.0000000, 3472.0000000, 4160.0000000);
		sys_test_equal(res, gt);
		...
		res = mat_33.conv(mat_22.flip_all_dim().clone());
		...
		gt = mt_mat_t<f32>(2, 2, 8).read(76.0000000, 84.0000000, 92.0000000, 100.0000000, 76.0000000, 84.0000000, 92.0000000, 100.0000000, 100.0000000, 112.0000000, 124.0000000, 136.0000000, 100.0000000, 112.0000000, 124.0000000, 136.0000000,
	...
	static void performance_test_channel_conv() {
		...
		sys_timer channel_conv("discontinuous_memory_conv");
		channel_conv.begin();
		...
		sys_timer single_conv("continuous_memory_conv");
		single_conv.begin();
...
static void test_save_mat() {
	...
	mt_mat_helper::save("test.txt", a);

	mt_mat b = mt_mat_helper::load("test.txt");

	sys_test_equal(a, b);

	mt_mat_helper::save("test.b", a, sys_false);

	b = mt_mat_helper::load("test.b", sys_false);
...
private:
	...
	static void test_mean_2d_channel_1() {
		...
		mt_mat b = a.pooling(mt_Pooling_Type_Mean, 2, kernel_sizes, strides);
		basiclog_info2(b);
	...
	static void test_meam_3d_channel_1() {
		...
		mt_mat b = a.pooling(mt_Pooling_Type_Mean, 3, kernel_sizes, strides);
		basiclog_info2(b);
	...
	static void test_max_2d_channel_1() {
		...
		mt_mat b = a.pooling(mt_Pooling_Type_Max, 2, kernel_sizes, strides);
		basiclog_info2(b);
	...
	static void test_max_3d_channel_n() {
		...
		mt_mat b = a.pooling(mt_Pooling_Type_Max, 3, kernel_sizes, strides);
		basiclog_info2(b);
	...
	static void test_max_sub_3d_channel_n() {
		...
		mt_mat b = sub_a.pooling(mt_Pooling_Type_Max, 3, kernel_sizes, strides);
	...
	static void test_performance_mean_2d_channel_1() {
		...
		sys_timer timer_1("src = 1000 * 1000, kernel = 2 * 2, stride = 2 * 2");
		timer_1.begin();
		...
		sys_timer timer_2("src = 1000 * 1000, kernel = 5 * 5, stride = 5 * 5");
		timer_2.begin();
	...
	static void test_performance_mean_3d_channel_1() {
		...
		sys_timer timer_1("src = 100 * 280 * 280, kernel = 1 * 5 * 5, stride = 1 * 5 * 5");
		timer_1.begin();
...
static void test_eigen() {
	...
	//basiclog_debug2("result:");
	//for(i32 i = 0; i < n; ++i) {
	//	basiclog_debug2(sys_strcombine() << "e" << i << " = " << ev.at<f32>(0, i, 1));
	//	sys_strcombine str;
	//	str << "v" << i  << " = (";
	//	for(i32 j = 0; j < n; ++j) {
	//		str << ec.at<f32>(i, j, 1);
...
void mt_mat_test::run(vector<string>& argvs) {
