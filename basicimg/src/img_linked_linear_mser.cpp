...
void img_linked_linear_mser::allocate_memory(const mt_mat& src, const img_mask_info<u8>& mask) {
	...
	i32 memory_cost = sizeof(i16) * m_masked_image_size + sizeof(i16*) * m_heap_size + sizeof(temp_linked_point) * m_point_size + sizeof(temp_mser_region) * m_region_size
...
void img_linked_linear_mser::extract_pixel_share_extraction_share_memory(img_multi_msers& msers, u8 gray_mask) {
	...
	//basiclog_info2(L"memory number: " <<memory_number);
	//ct.end();
}
